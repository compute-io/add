...
function add( x, y, options ) {
	...
	if ( isMatrixLike( x ) ) {
		if ( !isMatrixLike( y ) && !isNumber( y ) ) {
			throw new TypeError( 'add()::invalid input argument. Second argument must be matrix-like or a number primitive. Value: `' + x + '`.' );
		}
		if ( opts.copy !== false ) {
	...
	if ( isArrayLike( x ) ) {
		if ( !isArrayLike( y ) && !isNumber( y ) ) {
			throw new TypeError( 'add()::invalid input argument. Second argument must be array-like or a number primitive. Value: `' + x + '`.' );
		}
